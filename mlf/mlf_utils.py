...
def generate_xy(num_topics, num_days, keep_app=True, delay=60):
    ...
    keep_app : bool, optional
        Keep only application dates, otherwise granted. The default is True.
    delay : int, optional
    ...
    # define y output
    y = pd.DataFrame(index=X.index, columns=['y_ret', 'y_bin'], data=np.nan)
    ...
    # remove nans in y (some stocks were not publicly traded before some date)
